...
public:
     ...
     */
    static std::optional<Rect> resolveLogicalToPhysical(const std::vector<std::unique_ptr<PageBox>>& pages, uint32_t logical_offset, Twips page_gap, DocumentRegion region = DocumentRegion::Body, std::optional<size_t> target_page = std::nullopt);
